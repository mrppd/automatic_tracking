...
class Ui_AnnotationWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.entryRecallSc = QtWidgets.QShortcut(QtGui.QKeySequence('Ctrl+R'), self.window)
        self.entryRecallSc.activated.connect(self.recallPrevCoord)
        ...
        self.btnRecall = QtWidgets.QPushButton(self.verticalLayoutWidget)
        self.btnRecall.setObjectName("btnRecall")
        self.horizontalLayout_4.addWidget(self.btnRecall)
    ...
    def recallPrevCoord(self):
        ...
        filename = self.listImages.item(nextItemIndex).text()
        print(filename)
        tmpCoordsSelCurrent =  self.dfCoordsStorage.loc[(self.dfCoordsStorage['baseFolderName'] == self.baseFolderName) & 
                                                 (self.dfCoordsStorage['fileName'] == self.listImages.currentItem().text())] 
        self.fileNamesForDf = []
        self.indexForDf = []
        self.ptsTable.setRowCount(0)
        for index, row in tmpCoordsSelCurrent.iterrows():
            rowPosition = self.ptsTable.rowCount()
            self.ptsTable.insertRow(rowPosition)
            self.ptsTable.setItem(rowPosition , 0, QtWidgets.QTableWidgetItem(row['p1']))
            self.ptsTable.setItem(rowPosition , 1, QtWidgets.QTableWidgetItem(row['p2']))
        ...
        #self.paintEvent()
        #recalling previous coordinates
        tmpCoordsSel =  self.dfCoordsStorage.loc[(self.dfCoordsStorage['baseFolderName'] == self.baseFolderName) & 
                                                 (self.dfCoordsStorage['fileName'] == filename)]
        print(tmpCoordsSel)
        #self.fileNamesForDf = []
        #self.indexForDf = []
        #self.ptsTable.setRowCount(0)
        for index, row in tmpCoordsSel.iterrows():
            ...
            self.ptsTable.setItem(rowPosition , 2, QtWidgets.QTableWidgetItem(row['p3']))
            self.ptsTable.setItem(rowPosition , 3, QtWidgets.QTableWidgetItem(row['p4']))
            #self.fileNamesForDf.append(self.listImages.currentItem().text())
            #self.indexForDf.append(index)
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "Image Annotator (Version 1.5)"))  #Version 1.4: shortcut key added
        self.btnBack.setText(_translate("MainWindow", "<-Back"))							   #Version 1.5: recall added
        ...
        self.btnRecall.setText(_translate("MainWindow", "&Recall"))
        self.btnRecall.setToolTip('Ctrl+R to recall previous entry.')
        ...
        self.btnRecall.clicked.connect(self.recallPrevCoord)
