...
###
out = np.empty((total_sample, 4, int(img_tmp.shape[0]*scale_percent/100), int(img_tmp.shape[1]*scale_percent/100)), dtype=np.float32)

for i in range(total_sample):
  out_img = np.zeros((4, int(img_tmp.shape[0]*scale_percent/100), int(img_tmp.shape[1]*scale_percent/100)), dtype=np.float32)
...
###
#modelVAE.load_state_dict(torch.load(data_path+"/model_cross_mean.pyc"), strict=False)
...
model_128
torch.save(model_128.state_dict(), data_path+"/model_cross_mean_2.pyc")
    ...
    for i, (x, y) in enumerate(loader_test):
        ...
        for bt in range(len(x)):
            ...
            show_coordinates(x[bt].squeeze(dim=0).numpy(), y_hat[bt].cpu().numpy(), rad=7, threshold=0.0005, 
                             save_fig=False, load_ind=cnt, bt_ind=bt)
