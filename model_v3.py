...
for i in range(total_sample):
  ...
  for j in range(len(gr)):
    ...
    out_coor_tmp.append((int(gr.iloc[j].p1SY), int(gr.iloc[j].p1SX)))
    out_coor_tmp.append((int(gr.iloc[j].p2SY), int(gr.iloc[j].p2SX)))
    out_coor_tmp.append((int(gr.iloc[j].p3SY), int(gr.iloc[j].p3SX)))
    out_coor_tmp.append((int(gr.iloc[j].p4SY), int(gr.iloc[j].p4SX)))
...
def train(model,optimizer):
    ...
    for i, (x, y, _) in enumerate(loader_train):
...
def test(model,optimizer):
    ...
    with torch.no_grad():
        for i, (x, y, _) in enumerate(loader_test):
            x = x.to(device)
            y = y.to(device)
            current_batch_size = x.shape[0]
            # forward pass
...
###
modelVAE.load_state_dict(torch.load(data_path+"/model_cross_mean_5ch_output_3.pyc"), strict=False)
...
###
model_128 = train_and_evaluate_model(num_epochs=4000)
model_128
torch.save(model_128.state_dict(), data_path+"/model_cross_mean_5ch_output_3.pyc")
    ...
    for i in range(test_image[4].shape[0]):
        for j in range(test_image[4].shape[1]):
            ...
            if(ret>100):
                ...
                local_threshold = (local_threshold_sum / local_threshold_cnt)*0.00
    ...
    for i in range(given_points.shape[0]):
        ...
        for ind_acp, acp in enumerate(analysed_center_points):
            ...
            if(dist<radius):
                ...
                break;
    return(count_pred, given_pts.shape[0])
            
    

count_pred, count_given = countMatch(min_max_coor, given_pts)
print(count_pred, count_given)
...
VIDEO_IN_PATH = os.path.join(main_path, 'Pig_Videos/cam-5-ATW20181113-090713-1542096433.mp4')
VIDEO_OUT_PATH = os.path.join(main_path, 'Pig_Videos/Kamera-120170504-120223-1493892143_1slices/track_unknown3_slow1.mp4')
...
video_out = cv2.VideoWriter(VIDEO_OUT_PATH, cv2.VideoWriter_fourcc(*"MJPG"), 8.0, 
                            (1280, 400))
images = []
